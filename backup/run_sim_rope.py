...
def generate_trace(args, seqlen_list):
    ...
    #         filepath=f"../trace/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
    ...
    # else:
    #     filepath=f"../trace/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
    #     if not os.path.exists(filepath):
    ...
    for seqlen in seqlen_list:
        if args.model_parallel:          
            for FC_devices in FC_devices_list:
                ...
                filepath=f"../trace/{args.num_channels}_channels_per_device/model_parallel/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                if not os.path.exists(filepath):
                ...
                # filepath=f"../trace/{args.num_channels}_channels_per_device/model_parallel_FC/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                # if not os.path.exists(filepath):
            ...
            if channels_per_block < 1: #minimal_channel_per_block[args.model]:
                raise ValueError(f"Channels per block {channels_per_block} is less than minimal channel per block 1")
            filepath=f"../trace/{args.num_channels}_channels_per_device/pipeline_parallel/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
            if not os.path.exists(filepath):
    ...
    if len(expected_traces) > 0:
        ...
        combined_output_file = f"../cent_simulation/logs/{args.model}_all_traces_burst_stats.json"
...
def simulate_trace(args, seqlen_list):
    ...
    for seqlen in seqlen_list:
        if args.model_parallel:
            for FC_devices in FC_devices_list:
                for mode in ["model_parallel"]: #["model_parallel", "model_parallel_FC"]:
...
def load_data_point(args, seqlen, FC_devices, channels_per_block, PCIe_lanes_per_device, blocks_per_device, embedding_latency, utilized_devices, pp, tp):
    ...
    if args.model_parallel:
        ...
        embedding_latency_data = 0 #embedding_latency['model_parallel'][FC_devices]
    else:
        cxl_latency = vector_latency(embedding_size[args.model], PCIe_lanes_per_device)
        embedding_latency_data = 0 #embedding_latency['pipeline_parallel'][channels_per_block]
    acc_latency_dict = calculate_acc_latency(args, seqlen)

    acc_latency = (acc_latency_dict["RotEmbed_latency"]) * blocks_per_device #MAINUDDIN
    transformer_block_latency = pim_latency + cxl_latency + acc_latency #MAINUDDIN
    ...
    if args.model_parallel:
        ...
        for comp in energy_main.keys():
            ...
            power_alldv[comp] = (energy_main[comp] ) * pipeline_stages / stats["latency"]
    else:
        for comp in energy_main.keys():
...
def process_throughputs(args):
    ...
    if os.path.exists(args.processed_result_path):
        ...
        # Added both 'Channels per device' and 'Channels per block'
        columns = ['Model', 'Device number', 'Channels per device', 'Channels per block', 'Seqlen', 'Pipeline parallelism', 'Tensor parallelism', 'Phase', 'Total Latency (s)', 'Throughput (tokens/s)', 'Energy per Token (mJ)', 'Total power (W)']
        results_df = pd.DataFrame(columns=columns)
    ...
    if args.model_parallel:
        ...
        for FC_Devices in FC_devices_list:
            ...
            # STRICT FILTER: Added Device number to prevent averaging across different device counts
            base_df = df_simulation[
                (df_simulation['Model'] == args.model) & 
                (df_simulation['Device number'] == args.num_devices) & 
                (df_simulation['Pipeline parallelism'] == pp) & 
                (df_simulation['Tensor parallelism'] == tp)
            ]
            
            # STRICT GROUPING: Differentiates specific channel configurations
            for (cpd, cpb), df in base_df.groupby(['Channels per device', 'Channels per block']):
                ...
                new_result = {
                    'Model': args.model,
                    'Device number': args.num_devices,
                    'Channels per device': cpd,
                    'Channels per block': cpb,
                    'Seqlen': args.prefill + args.decoding,
                    'Pipeline parallelism': pp,
                    'Tensor parallelism': tp,
                    'Phase': args.phase,
        ...
        # STRICT FILTER: Added Device number 
        base_df = df_simulation[
            (df_simulation['Model'] == args.model) & 
            (df_simulation['Device number'] == args.num_devices) & 
            (df_simulation['Pipeline parallelism'] == TransformerBlock_number[args.model]) & 
            (df_simulation['Tensor parallelism'] == 1)
        ]
        ...
        for (cpd, cpb), df in base_df.groupby(['Channels per device', 'Channels per block']):
            ...
            average_throughput = df['Throughput (tokens/s)'].mean()
            average_energy = df['Token energy (mJ)'].mean()
            total_latency = df['Token latency (ms)'].mean() * (seqlen/4) / 1000
            total_power = df['Total power (W)'].mean()
    ...
    # OVERWRITE FIX: Added subset parameters and forced keep='last' to ensure new runs override old runs
    subset_cols = ['Model', 'Device number', 'Channels per device', 'Channels per block', 'Seqlen', 'Pipeline parallelism', 'Tensor parallelism', 'Phase']
    results_df = results_df.drop_duplicates(subset=subset_cols, keep='last')
    results_df = results_df.sort_values(by=subset_cols)
