...
class TransformerBlockLlama(TransformerBlock):
    ...
    def self_attention_aim(self):
        ...
        print("Entered self_attention_aim in Llama.py")
        ...
        print("Entered self_attention_aim in Llama.py, : trace_fc_kqvo is true, model name ={} ".format(self.model))

        #MAINUDDIN
        # self.apply_rotary_emb_pim_only_trace(channel_lst_multi_transformer_block, channels_required, self.xq_row_index, self.xk_row_index, self.trace_fc_kqvo) #MAINUDDIN
        if self.trace_fc_kqvo:
            ...
            self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xq_row_index, input_vector_EWMUL_length * 2)
            self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            ...
            self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xq_row_index, input_vector_EWMUL_length * 2)

            self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
    ...
    def trace_only(self):
        ...
        print("Entered trace only in Llama.py")
        ...
        if self.trace_fc_kqvo:
            ...
            print("Entered trace only in Llama.py: trace_fc_kqvo is true, model name ={} ".format(self.model))
            print("Model name ={} with model parallel enable: {},  Dimension ={}, Total banks ={}".format(self.model, self.model_parallel, self.dim, self.total_banks))
            ...
            self.rd_q_burst=self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xq_row_index, input_vector_EWMUL_length * 2)
            self.store_for_rd_q_burst=self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xq_row_index, input_vector_EWMUL_length * 2)
            self.rd_k_burst=self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            self.store_for_rd_k_burst=self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 0, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            ...
            # Load rotary embedding results
            self.inv_q_burst=self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xq_row_index, input_vector_EWMUL_length * 2)
            self.store_for_inv_q_burst=self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xq_row_index, input_vector_EWMUL_length * 2)

            self.inv_k_burst=self.COPYGB_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
            self.store_for_inv_k_burst=self.COPYBK_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
    ...
    # #MAINUDDIN
    # def COPYGB_for_EWMUL_input_only_trace(self, ind, channel_lst_multi_transformer_block, utilized_banks_per_channel, bank_grp_index, row_index, base_op_size):
    #     wr_q_burst_l=0;
    #     for bank in range(utilized_banks_per_channel):
    #         print("CopyGB {}: Bank {}, Row index {}, Op size {}".format(ind, bank*4+bank_grp_index, row_index, base_op_size))
    #         self.COPY_MIN_GB_only_trace(channel_lst_multi_transformer_block, bank*4+bank_grp_index, row_index, base_op_size)  
    #         ##INPUT Bank=  : bank_group_index=0
    ...
    #         print("CopyGB {}: Bank {}, Total burst till now: {}".format(ind, bank*4+bank_grp_index, wr_q_burst_l))

    # #total_channels * utilized_banks_per_channel #authors
    # #num_transformer_blocks_per_device * channels_required * utilized_banks_per_channel #equivalence
    # #num_transformer_blocks_per_device * input_vector_EWMUL_utilized_banks #mine
    ...
    #     print("")
    #     print("EQUIVALENCE author={}, intermediate={}, mine={}".format(total_channels * utilized_banks_per_channel,num_transformer_blocks_per_device * channels_required * utilized_banks_per_channel, num_transformer_blocks_per_device * input_vector_EWMUL_utilized_banks))
    #     print("")
    ...
    #     # Store re-mapped xq/xk for EWMUL
    #     self.COPYGB_for_EWMUL_input_only_trace("RD Q", channel_lst_multi_transformer_block, utilized_banks_per_channel, 0, self.xq_row_index, base_op_size)
    #     #self.time["WR_SBK"] += self.timing_constant["WR_SBK"] + self.dim * 2 // self.burst_length
    #     self.rd_q_burst=total_channels*utilized_banks_per_channel*input_vector_EWMUL_length * 2
    ...
    #     self.store_for_rd_q_burst=num_transformer_blocks_per_device*input_vector_EWMUL_utilized_banks*input_vector_EWMUL_length * 2 

    #     self.COPYGB_for_EWMUL_input_only_trace("RD K", channel_lst_multi_transformer_block, utilized_banks_per_channel, 0, self.xk_row_index, base_op_size // self.n_repeat)
    #     #self.time["WR_SBK"] += self.timing_constant["WR_SBK"] + self.dim * 2 // self.burst_length
    #     self.rd_k_burst=total_channels*utilized_banks_per_channel*input_vector_EWMUL_length * 2 // self.n_repeat 
    ...
    #     self.store_for_rd_k_burst=num_transformer_blocks_per_device*input_vector_EWMUL_utilized_banks*input_vector_EWMUL_length // self.n_repeat * 2 

    #     # Rotary embedding
    #     self.EWMUL_only_trace(channel_lst_multi_transformer_block, self.xq_row_index, self.dim // self.burst_length)
    #     self.ewmul_q_burst=num_transformer_blocks_per_device*self.dim 

    #     self.EWMUL_only_trace(channel_lst_multi_transformer_block, self.xk_row_index, self.dim // self.n_repeat // self.burst_length)
    #     self.ewmul_k_burst=num_transformer_blocks_per_device*self.dim // self.n_repeat 
        
    #     # Load rotary embedding results
    #     self.COPYGB_for_EWMUL_input_only_trace("RD INV Q", channel_lst_multi_transformer_block, utilized_banks_per_channel, 2, self.xq_row_index, base_op_size)
    #     #self.time["RD_SBK"] += self.timing_constant["RD_SBK"] + self.dim * 2 // self.burst_length
    #     self.inv_q_burst=total_channels*utilized_banks_per_channel*input_vector_EWMUL_length * 2
        
    #     self.store_for_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xq_row_index, input_vector_EWMUL_length * 2)
    #     self.store_for_inv_q_burst=num_transformer_blocks_per_device*input_vector_EWMUL_utilized_banks*input_vector_EWMUL_length * 2 

    #     self.COPYGB_for_EWMUL_input_only_trace("RD INV K", channel_lst_multi_transformer_block, utilized_banks_per_channel, 2, self.xk_row_index, base_op_size // self.n_repeat )
    #     #self.time["RD_SBK"] += self.timing_constant["RD_SBK"] + self.dim * 2 // self.burst_length
    #     self.inv_k_burst=total_channels*utilized_banks_per_channel*input_vector_EWMUL_length * 2 // self.n_repeat 
        
    #     self.store_for_EWMUL_input_only_trace(channels_required, input_vector_EWMUL_utilized_banks, 2, self.xk_row_index, input_vector_EWMUL_length // self.n_repeat * 2)
    #     self.store_for_inv_k_burst=num_transformer_blocks_per_device*input_vector_EWMUL_utilized_banks*input_vector_EWMUL_length // self.n_repeat * 2 
    ...
    #     print("Model: {}, RD-WR SBK for Q and K is same={}, ewmul_q_burst={}, ewmul_k_burst={}".format(self.model, self.dim * 2 , self.dim , self.dim // self.n_repeat ))
