...
class TransformerBlock(PIM):
    ...
    def store_for_EWMUL_input_only_trace(self, channels_required, total_banks, bank_group_index, row_index, size):
        #num_transformer_blocks_per_device is number of different unrelated Transformer blocks that we can put on single CXL device
        #As there are 32 channels, having 4 blocks would mean
        #Block 0: uses channels{0-7}
        #Block 1: uses channels{8-15}
        #Block 2: uses channels{16-23}
        #Block 3: uses channels{24-31}
        num_transformer_blocks_per_device = max(self.num_channels // channels_required, 1)
        #print("ENTER store_for_EWMUL_input_only_trace, num_transformer_blocks_per_device= {}, size={}, burst_length= {}".format(num_transformer_blocks_per_device, size, self.burst_length  ))
        #print("ENTER store_for_EWMUL_input_only_trace, channels_required= {}, bank_group_index={}".format(channels_required, bank_group_index))
        for i in range((size - 1) // self.burst_length + 1):
            #print("\tstore_for_EWMUL_input_only_trace, i= {}, i is burst size???".format(i))
            for bank in range(total_banks):
                #print("\t\tstore_for_EWMUL_input_only_trace, bank= {}".format(bank))
                dimm_index, channel_index, bank_index = self.bank_index(bank*4+bank_group_index)
                #print("\t\tstore_for_EWMUL_input_only_trace, DIMM base index= {}, channel base index= {}, bank base index= {}".format(dimm_index, channel_index, bank_index ))
                for tb in range(num_transformer_blocks_per_device):
                    self.W_MEM_only_trace(channel_index + channels_required * tb, bank_index, row_index, self.burst_length)
                    #print("\t\t\tTB loop store_for_EWMUL_input_only_trace, channel_accessed= {}".format(channel_index + channels_required * tb))
    ...
    def COPYBK_EWMUL_input_only_trace(self, channels_required, input_vector_EWMUL_utilized_banks, bank_group_index, row_index, size): #MAINUDDIN
        wr_burst=0;
        num_transformer_blocks_per_device = max(self.num_channels // channels_required, 1)
        ...
        for bank in range(utilized_banks_per_channel):
            for tb in range(num_transformer_blocks_per_device):
                channel_lst=list( range(channels_required*tb, channels_required*(tb+1)) ) #channels used in current tb
                ...
                wr_burst=wr_burst+(len(channel_lst)*opsize)
        return wr_burst

    def COPYGB_EWMUL_input_only_trace(self, channels_required, input_vector_EWMUL_utilized_banks, bank_group_index, row_index, size): #MAINUDDIN
        rd_burst=0;
        num_transformer_blocks_per_device = max(self.num_channels // channels_required, 1)
        utilized_banks_per_channel=input_vector_EWMUL_utilized_banks//channels_required #for channel parallelism
        opsize= (size - 1) // self.burst_length + 1 #opsize is sent, not size
        for bank in range(utilized_banks_per_channel):
            for tb in range(num_transformer_blocks_per_device):
                channel_lst=list( range(channels_required*tb, channels_required*(tb+1)) ) #channels used in current tb
                self.COPY_MIN_GB_only_trace(channel_lst, bank+bank_group_index, row_index, opsize)  #channel list is sent, not number of channels
                rd_burst=rd_burst+(len(channel_lst)*opsize)
        return rd_burst;
    ...
    #         self.COPY_MIN_GB_only_trace(channel_lst_multi, bank+bank_group_index, row_index, opsize)  #channel list is sent, not number of channels
    ...
    #MAINUDDIN
    def COPY_GB_BK_for_EWMUL_input_only_trace(self, channels_required, total_banks, bank_group_index, row_index, size):
        num_transformer_blocks_per_device = max(self.num_channels // channels_required, 1)
        for i in range((size - 1) // self.burst_length + 1):
            for bank in range(total_banks):
                dimm_index, channel_index, bank_index = self.bank_index(bank*4+bank_group_index)
                for tb in range(num_transformer_blocks_per_device):
                    self.COPY_GB_BK_only_trace(channel_index + channels_required * tb, bank_index, row_index, self.burst_length)
