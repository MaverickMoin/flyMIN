...
def extract_latencies(df_source, model, device, transformer_blocks, tps):
    ...
    for tp in tps:
        ...
        print(df['Channels per device'],"--",df['Sequence length'])
...
for model, device_numbers in model_device_combinations.items():
    for device in device_numbers:
        ...
        df_results = pd.DataFrame({'Config': parallel_list,
            'Baseline PIM': pim_b, 'Baseline Acc': acc_b, # Update component names
            'Mount PIM': pim_m,  'Mount Acc': acc_m, 
            'Fly PIM': pim_f, 'Fly Acc': acc_f
        })
        ...
        sources = [
            ("CENT Baseline", pim_b, acc_b, y_pos - bar_height, ''),
            ("mountMIN", pim_m, acc_m, y_pos, '///'), # case sensitive target name match to image
            ("flyMIN", pim_f, acc_f, y_pos + bar_height, '\\\\\\') # case sensitive target name match
        ]
        ...
        for label, pim, acc, pos, hatch in sources:
            ...
            ax.barh(pos, acc, left=pim, height=bar_height, color="darkgreen", edgecolor='black', hatch=hatch, label="") 
        ...
        legend_patches = [pim_patch, acc_patch, baseline_hatch, mountmin_hatch, flymin_hatch]
        ...
        plt.savefig(f'figures/rope_figure_{model}_{device}_14c.pdf')
        plt.close()