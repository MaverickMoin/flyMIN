...
# Removed minimal_channel_per_block from the import below
from utils import InOut_latency, n_heads, gqa_factor, embedding_size, ffn_size, TransformerBlock_number, pipeline_parallel_mode_list, model_parallel_mode_list

import json

# Integrated minimal_channel_per_block
minimal_channel_per_block = {"Llama2-7B": 1, "Llama2-13B": 1, "Llama2-70B": 1}
...
#MAINUDDIN
def log_command(model, embedding, onlyFC, onlyTrace,modelParallel,multiTBperDevice,FC_devices_list,channels_per_block):
    # Determine parallelism state
    
    log_data = {
        "model_name": args.model,
        #"model_obj": str(model),
        "n_heads": n_heads[args.model],
        #"ffn_dim": ffn_size[args.model],
        "num_channels": args.num_channels,
        "channels_per_block": channels_per_block if not modelParallel else "N/A",
        "fc_devices": FC_devices_list,
        "reuse_size": args.reuse_size,
        "embedding": embedding,
        "only-FC": onlyFC,
        "only-trace": onlyTrace,
        "model_parallel": modelParallel,
        "pipeline_parallel": not modelParallel,
        "multi_tb_per_device": multiTBperDevice,
        #"op-trace": True,
        #"GEMV": True,
        #"reuse-GB": True,
    }
...
def generate_trace(args, seqlen_list):
    ...
    print("\nGenerate trace in run_sim, model name={} sending as model {}\n".format(args.model, model))

    commands_generate_traces = []
    expected_traces = {} #MAINUDDIN
    ...
    if args.model_parallel:
        for FC_devices in FC_devices_list:
            filepath=f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
            if not os.path.exists(filepath):
                commands_generate_traces.append(["python3", "function_sim.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--embedding", "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                config_dict =log_command(model, True, False, True, True, False,FC_devices_list,0) #MAINUDDIN
                expected_traces[filepath] = config_dict    
    else:
        filepath=f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
        if not os.path.exists(filepath):
            commands_generate_traces.append(["python3", "function_sim.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--embedding", "--only-trace", "--num-channels", str(args.num_channels), "--channels-per-block", str(channels_per_block), "--pipeline-parallel", "--multi-tb-per-device", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
            config_dict =log_command(model, True, False, True, False, True,[],channels_per_block) #MAINUDDIN
            expected_traces[filepath] = config_dict
    ...
    for seqlen in seqlen_list:
        if args.model_parallel:          
            for FC_devices in FC_devices_list:
                ...
                filepath=f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                if not os.path.exists(filepath):
                    commands_generate_traces.append(["python3", "function_sim.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                    if args.inter_device_attention:
                        commands_generate_traces[-1].append("--inter-device-attention")
                    config_dict =log_command(model, False, False, True, True, False,FC_devices_list,0) #MAINUDDIN
                    expected_traces[filepath] = config_dict

                filepath=f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_FC/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                if not os.path.exists(filepath):
                    commands_generate_traces.append(["python3", "function_sim.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-FC", "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                    config_dict =log_command(model, False, True, True, True, False,[],0) #MAINUDDIN
                    expected_traces[filepath] = config_dict 
        else:
            ...
            filepath=f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
            if not os.path.exists(filepath):
                commands_generate_traces.append(["python3", "function_sim.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-trace", "--num-channels", str(args.num_channels), "--channels-per-block", str(channels_per_block), "--pipeline-parallel", "--multi-tb-per-device", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                config_dict =log_command(model, False, False, True, False, True,[],channels_per_block) #MAINUDDIN
                expected_traces[filepath] = config_dict          
    ...
    # MAINUDDIN
    print("current run_sim.py directory files after trace generation: ", os.getcwd())
    if len(expected_traces) > 0:
        combined_results = []

        # Loop directly over expected_traces because its keys are the perfectly formatted file paths!
        for trace_file_path, config_data in expected_traces.items():
            stats_file_path = f"{trace_file_path}.json"
            ...
            if os.path.exists(stats_file_path):
                ...
                print(f"Warning: Expected JSON stat file missing: {stats_file_path}")

            # Package the config and stats together
            combined_results.append({
                "traces": trace_file_path,
                **config_data,
                **run_stats
            })
        
        combined_output_file = f"../cent_simulation/logs/{args.num_devices}_devices_{args.model}_all_traces_burst_stats.json"
        with open(combined_output_file, "w") as f:
            json.dump(combined_results, f, indent=4)

        print(f"\n=== NEW TRACES GENERATED {len(expected_traces)} . COMBINED STATS SAVED TO: {combined_output_file} ===")
        
    else:
        print("\n=== ALL TRACES ALREADY EXIST. NO NEW TRACES GENERATED. SKIPPING JSON UPDATE. ===")
    print("=================================================================\n")
...
def run_command(command, log_file):
    ...
    # Filter standard output as before
    filtered_output = "\n".join(
        line for line in result.stdout.splitlines() if not line.startswith('[')
    )
    
    with open(log_file, "w") as log:
        # 1. Write the filtered stdout
        log.write(filtered_output)
        
        # 2. If there is an error, append it to the log file
        if result.returncode != 0:
            log.write("\n" + "="*20 + " ERROR LOG " + "="*20 + "\n")
            log.write(f"Command failed with exit code: {result.returncode}\n")
            log.write("Error Message:\n")
            log.write(result.stderr)
            
            # Also print to console so you see it immediately
            print(f"Error executing command. Check {log_file} for details.")

def detect_emtpy_file(file):
    return os.stat(file).st_size == 0

def simulate_trace(args, seqlen_list):
    ramulator_build_path = os.path.abspath("../aim_simulator")
    if 'LD_LIBRARY_PATH' in os.environ:
        os.environ['LD_LIBRARY_PATH'] = f"{ramulator_build_path}:{os.environ['LD_LIBRARY_PATH']}"
    else:
        os.environ['LD_LIBRARY_PATH'] = ramulator_build_path
    ...
    if args.model_parallel:
        for FC_devices in FC_devices_list:
            log_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
            if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                trace_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
        ...
        log_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
        if not os.path.exists(log_file) or detect_emtpy_file(log_file):
            trace_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
            command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
            commands_simulate_traces.append((command, log_file))
        ...
        if args.model_parallel:
            for FC_devices in FC_devices_list:
                for mode in ["model_parallel", "model_parallel_FC"]:
                    log_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
                    if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                        trace_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                        command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
                        commands_simulate_traces.append((command, log_file))
        else:
            for mode in ["pipeline_parallel"]:
                log_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
                if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                    trace_file = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
                    command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
                    commands_simulate_traces.append((command, log_file))
...
def process_results(args):
    ...
    for mode in mode_list:
        compile_dir = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/"
...
def load_data_point(args, seqlen, FC_devices, channels_per_block, PCIe_lanes_per_device, blocks_per_device, embedding_latency, utilized_devices, pp, tp):
    ...
    if args.model_parallel:
        path = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
    else:
        path = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
    ...
    if args.model_parallel:
        ...
        FC_path = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_FC/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
...
def update_csv(args, seqlen_list):
    ...
    if args.model_parallel:
        FC_devices_list = factorize(args.num_devices)
        for FC_devices in FC_devices_list:
            embedding_compile_dir = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/"
        ...
        embedding_compile_dir = f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/"
        with open(f"{embedding_compile_dir}/compiled_results.txt", "r") as compiled_results_file:
            lines = compiled_results_file.readlines()
            for line in lines:
                filename, latency = line.split()[0], line.split()[1]
...
def process_throughputs(args):
    ...
    if args.model_parallel:
        ...
        for FC_Devices in FC_devices_list:
            ...
            pp = args.num_devices // FC_Devices
            tp = FC_Devices
            # FIXED: Added Device number filter
            df = df_simulation[(df_simulation['Model'] == args.model) & (df_simulation['Device number'] == args.num_devices) & (df_simulation['Pipeline parallelism'] == pp) & (df_simulation['Tensor parallelism'] == tp)]
            # print("tp", tp, len(df))
        ...
        # FIXED: Added Device number filter
        df = df_simulation[(df_simulation['Model'] == args.model) & (df_simulation['Device number'] == args.num_devices) & (df_simulation['Pipeline parallelism'] == TransformerBlock_number[args.model]) & (df_simulation['Tensor parallelism'] == 1)]
...
if __name__ == "__main__":
    ...
    for mode in pipeline_parallel_mode_list + model_parallel_mode_list:
        subprocess.run(["mkdir", "-p", f"../trace/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}"])
