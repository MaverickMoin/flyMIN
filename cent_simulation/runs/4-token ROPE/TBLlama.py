...
class TransformerBlockLlama(TransformerBlock):
    ...
    def trace_only(self):
        ...
        input_vector_EWMUL_length = (dim - 1) // (total_banks // 4) + 1
        input_vector_EWMUL_utilized_banks = (dim - 1) // input_vector_EWMUL_length + 1
        ...
        if self.trace_fc_kqvo:
            ...
            # Rotary embedding
            self.EWMUL_only_trace(channel_lst, self.xq_row_index, dim // self.burst_length)
            self.EWMUL_only_trace(channel_lst, self.xk_row_index, dim // self.n_repeat // self.burst_length)
