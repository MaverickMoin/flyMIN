...
class PIM():
    ...
    def COPY_GB_BK_only_trace(self, channel, bank, row_index, op_size):
        assert bank < self.num_banks
        self.time["COPY_GB_BK"] += self.timing_constant["COPY_GB_BK"] + op_size
        print("How COPY_GB_BK_only_trace hexadecimal channel mask looks for {} channels: {}".format(channel, self.hex_channel_mask(channel)))
        self.file.write("AiM COPY_GBBK {} {} {} {}\n".format(op_size, self.hex_channel_mask(channel), bank, row_index))
    ...
    #MAINUDDIN
    def MULINT(self, dimm, channel, utilized_channels, col_index, op_size, op_trace):
        """
        ...
        if op_trace and dimm == 0:
            channel_multi_transformer_block_required = self.num_channels // utilized_channels * utilized_channels
            channel_lst = [c for c in range(channel_multi_transformer_block_required)]
            self.file.write("AiM ISR_MIN {} {} 0\n".format(op_size, self.hex_channel_mask(channel_lst)))

        # 2. Functional Simulation (PyTorch)
        # If we are not in only_trace mode, actually shuffle the data in the Global Buffer

        if hasattr(self, 'pim_device') and self.pim_device:
            # Load the active chunk from the Global Buffer
            gb_data = self.pim_device["dimm_" + str(dimm)].dimm["channel_" + str(channel)].GB[col_index : col_index + op_size * self.burst_length]
            
            # Execute MIN permutation for RoPE complex pairing: [x0, x1, x2, x3] -> [-x1, x0, -x3, x2]
            # Reshape into pairs, swap, and negate the first element to mimic the complex plane rotation prep
            pairs = gb_data.view(-1, 2)
            
            # Swap: [x, y] -> [-y, x]
            # column 0 gets -1 * column 1
            # column 1 gets column 0
            shuffled_data = torch.stack([-pairs[:, 1], pairs[:, 0]], dim=1).flatten()
            
            # Write the shuffled data back to the Global Buffer
            self.pim_device["dimm_" + str(dimm)].dimm["channel_" + str(channel)].GB[col_index : col_index + op_size * self.burst_length] = shuffled_data
    ...
    def MULINT_only_trace(self, channel, op_size):
        ...
        self.time["MULINT"] += self.timing_constant["MULINT"] + op_size
        print("How the hexadecimal channel mask looks for {} channels: {}".format(channel, self.hex_channel_mask(channel)))
        # Write the trace instruction. Format: "AiM MULINT <op_size> <channel_mask> <dummy_row>"
        self.file.write("AiM ISR_MIN {} {}\n".format(op_size, self.hex_channel_mask(channel)))
        
    def COPY_MIN_GB_only_trace(self, channel, bank, row_index, op_size):
        assert bank < self.num_banks
        self.time["MULINT"] += self.timing_constant["MULINT"] + op_size
        print("How COPY_MIN_GB_only_trace hexadecimal channel mask looks for {} channels: {}".format(channel, self.hex_channel_mask(channel)))
        self.file.write("AiM ISR_MIN_GB {} {} {} {}\n".format(op_size, self.hex_channel_mask(channel), bank, row_index))
