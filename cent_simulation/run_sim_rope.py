...
def get_args():
    ...
    parser.add_argument("--simulation_result_path", type=str, help="Path to the result file", default="ropesimulation_results.csv")
    parser.add_argument("--process_throughputs", action="store_true", help="average throughputs for various seqlen")
    parser.add_argument("--processed_result_path", type=str, help="Path to the final result file", default="ropeprocessed_results.csv")
...
def generate_trace(args, seqlen_list):
    ...
    if args.model_parallel:
        for FC_devices in FC_devices_list:
            filepath=f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
            if not os.path.exists(filepath):
                commands_generate_traces.append(["python3", "function_sim_rope.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--embedding", "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                config_dict =log_command(model, True, False, True, True, False,FC_devices_list,0) #MAINUDDIN
                expected_traces[filepath] = config_dict    
    else:
        filepath=f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
        if not os.path.exists(filepath):
            commands_generate_traces.append(["python3", "function_sim_rope.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--embedding", "--only-trace", "--num-channels", str(args.num_channels), "--channels-per-block", str(channels_per_block), "--pipeline-parallel", "--multi-tb-per-device", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
    ...
    for seqlen in seqlen_list:
        if args.model_parallel:          
            for FC_devices in FC_devices_list:
                ...
                filepath=f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                if not os.path.exists(filepath):
                    commands_generate_traces.append(["python3", "function_sim_rope.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
                ...
                filepath=f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_FC/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                if not os.path.exists(filepath):
                    commands_generate_traces.append(["python3", "function_sim_rope.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-FC", "--only-trace", "--num-channels", str(args.num_channels), "--FC-devices", str(FC_devices), "--model-parallel", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
            ...
            filepath=f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
            if not os.path.exists(filepath):
                commands_generate_traces.append(["python3", "function_sim_rope.py", model, "--n_heads", str(n_heads[args.model]), "--ffn_dim", str(ffn_size[args.model]), "--only-trace", "--num-channels", str(args.num_channels), "--channels-per-block", str(channels_per_block), "--pipeline-parallel", "--multi-tb-per-device", "--seqlen", str(seqlen), "--op-trace", "--GEMV", "reuse-GB", "--reuse-size", str(args.reuse_size), "--trace-file", filepath])
    ...
    if len(expected_traces) > 0:
        ...
        combined_output_file = f"../cent_simulation/logs_rope/{args.num_devices}_devices_{args.model}_all_traces_burst_stats.json"
...
def simulate_trace(args, seqlen_list):
    ...
    if args.model_parallel:
        for FC_devices in FC_devices_list:
            log_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
            if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                trace_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
        ...
        log_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
        if not os.path.exists(log_file) or detect_emtpy_file(log_file):
            trace_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
            command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
            commands_simulate_traces.append((command, log_file))
        ...
        if args.model_parallel:
            for FC_devices in FC_devices_list:
                for mode in ["model_parallel", "model_parallel_FC"]:
                    log_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
                    if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                        trace_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt"
                        command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
                        commands_simulate_traces.append((command, log_file))
        else:
            for mode in ["pipeline_parallel"]:
                log_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
                if not os.path.exists(log_file) or detect_emtpy_file(log_file):
                    trace_file = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt"
                    command = f"../aim_simulator/build/ramulator2 -f ../aim_simulator/test/example.yaml -t {trace_file}"
                    commands_simulate_traces.append((command, log_file))
...
def process_results(args):
    ...
    for mode in mode_list:
        compile_dir = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}/"
        subprocess.run(["cp", "../trace_rope/compile.sh", compile_dir])
        subprocess.run(["cp", "../trace_rope/compile.py", compile_dir])
...
def load_data_point(args, seqlen, FC_devices, channels_per_block, PCIe_lanes_per_device, blocks_per_device, embedding_latency, utilized_devices, pp, tp):
    ...
    if args.model_parallel:
        path = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
    else:
        path = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel/{args.model}/trace_{channels_per_block}_channels_per_block_seqlen_{seqlen}.txt.log"
    ...
    acc_latency = (acc_latency_dict["RotEmbed_latency"]) * blocks_per_device
    print(f"acc_latency: {acc_latency} , RotEmbed_latency: {acc_latency_dict['RotEmbed_latency']}")
    ...
    if args.model_parallel:
        ...
        FC_path = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_FC/{args.model}/trace_{FC_devices}_FC_devices_seqlen_{seqlen}.txt.log"
...
def update_csv(args, seqlen_list):
    ...
    if args.model_parallel:
        FC_devices_list = factorize(args.num_devices)
        for FC_devices in FC_devices_list:
            embedding_compile_dir = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/model_parallel_embedding/{args.model}/"
        ...
        embedding_compile_dir = f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/pipeline_parallel_embedding/{args.model}/"
        with open(f"{embedding_compile_dir}/compiled_results.txt", "r") as compiled_results_file:
            lines = compiled_results_file.readlines()
            for line in lines:
                filename, latency = line.split()[0], line.split()[1]
...
if __name__ == "__main__":
    ...
    for mode in pipeline_parallel_mode_list + model_parallel_mode_list:
        subprocess.run(["mkdir", "-p", f"../trace_rope/{args.num_devices}_devices/{args.num_channels}_channels_per_device/{mode}/{args.model}"])
